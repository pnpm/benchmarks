...
Its cache is filled once per run, by a job of its own. The warm-up is the slowest thing a run does — the current fixtures come to some 5.4k packages and ~3 GB — and it is identical work in every sample job. `pnpm run populate-cache` (`populatePnprCache.js`) does it alone and leaves the storage in `.pnpr-cache`, which the workflow shares with the three measuring jobs through the run's own cache key; each of them points `BENCHMARK_PNPR_CACHE` at it and skips the warm-up. Three jobs warming their own registry is not merely three times the load on the upstreams: it is three *different* answers whenever a package publishes mid-run, and the samples stop being samples of the same thing. A run whose cache didn't arrive warms the registry itself, so a miss costs time rather than numbers.

The populate pass deliberately does not go through the emulated link. Nothing it does is timed, and pulling thousands of packages through a rate-limited proxy while pnpr fetches from its upstreams is both pointless and, at the size of the larger fixture, where the warm-up wedged: a hundred sockets open on the link and no progress.

Packages that are not on npmjs are routed by the server, not by the clients. Bit publishes `@teambit/*` and `@bitdev/*` to a registry of its own, and `alotta-packages` depends on them; the config declares that registry alongside npmjs and puts a `router` in front of both, which resolves each name to the first source whose declared packages claim it. `defaultRegistry` names the router rather than a concrete registry — a concrete one resolves only its own namespace and answers "unclaimed" for everything else, with no fall-through to a sibling. Every manager therefore keeps installing from the single registry URL it is handed, with no scoped-registry configuration of its own, and bit's tarballs cross the emulated link like every other package's.
